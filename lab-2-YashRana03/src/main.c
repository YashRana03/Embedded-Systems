...
int c = 0; // This variable is the counter variable that makes sure current led changes after 1s
...
void moveLit() {
	...
	if(c > 0) c--; // Decrements the counter if it is higher than 0
	...
	if(c == 0) {
		...
		// this checks if the current direction of the movement and depending on that icrements the currentLed or decrements it
		if(direction) {
			currentLed++;
		...
		// it switches the currentLed to be the oppsite of that 
		if(stateLEDS == LED_OFF) {
			ledOnOff(LEDS[currentLed], LED_ON);
		}
		else ledOnOff(LEDS[currentLed], LED_OFF);
